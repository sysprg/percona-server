...
typedef struct
{
  char name[USERNAME_LENGTH + HOSTNAME_LENGTH];
  size_t length;
} account;
...
account *account_create(const char *user, size_t user_length,
                        const char *host, size_t host_length)
{
  account *acc= (account *) my_malloc(sizeof(account),
...
*/
static
void unqote_string(char *string, size_t *string_length)
{
...
void account_list_from_string(HASH *hash, const char *string)
{
  ...
  my_bool quote;
  char user[USERNAME_LENGTH], host[HOSTNAME_LENGTH];
  ...
  while (entry - string_copy < string_length)
  {
    ...
    unqote_string(user, &user_length);
    unqote_string(host, &host_length);

    my_hash_insert(hash,
        (uchar*) account_create(user, user_length, host, host_length));

    entry= entry + entry_length + 1;
  }
...
void audit_log_filter_init()
{
  ...
  mysql_rwlock_register("server_audit", all_rwlock_list,
